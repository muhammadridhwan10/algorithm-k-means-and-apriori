...
function jumlah_data_latih($db_object, $where=null){
	$sql = "SELECT COUNT(*) FROM data_uji ".$where;
 ...
 * @return array
 */
function ProsesNaiveBayes($db_object, $id_data_uji=0, 
        $mar, $apr, $mei, $show_perhitungan=true){
	...
	$x_total_apr_tinggi = $jumlah_total_mar_tinggi/$jumlah_tinggi;
	//x jawaban_a  koleris
	...
	$belakang_mar_tinggi = exp( ((pow($usia-$x_total_mar_tinggi,2)) / (2*$s2_pangkat2_mar_tinggi)));
	$prob_mar_tinggi = 1/($depan_mar_tinggi * $belakang_mar_tinggi);
        //koleris
	$depan_mar_rendah = sqrt($dua_phi*$s2_total_mar_rendah);
	$belakang_mar_rendah = exp( ((pow($usia-$x_total_mar_rendah,2)) / (2*$s2_pangkat2_mar_rendah)));
	...
	$belakang_apr_tinggi = exp( ((pow($usia-$x_total_apr_tinggi,2)) / (2*$s2_pangkat2_apr_tinggi)));
	$prob_apr_tinggi = 1/($depan_apr_tinggi * $belakang_apr_tinggi);
        //koleris
	$depan_apr_rendah = sqrt($dua_phi*$s2_total_apr_rendah);
	$belakang_apr_rendah = exp( ((pow($usia-$x_total_apr_rendah,2)) / (2*$s2_pangkat2_apr_rendah)));
	...
	$belakang_mei_tinggi = exp( ((pow($usia-$x_total_mei_tinggi,2)) / (2*$s2_pangkat2_mei_tinggi)));
	$prob_mei_tinggi = 1/($depan_mei_tinggi * $belakang_mei_tinggi);
        //koleris
	$depan_mei_rendah = sqrt($dua_phi*$s2_total_mei_rendah);
	$belakang_mei_rendah = exp( ((pow($usia-$x_total_mei_rendah,2)) / (2*$s2_pangkat2_mei_rendah)));
	...
	// echo "P(mar|Rendah)=".number_format($prob_mei_rendah, dec())."<br>";
        ...
        //===============================
	$nilai_tinggi = $p_tinggi * $prob_mar_tinggi * $prob_apr_tinggi *
					$prob_mei_tinggi;
        if($show_perhitungan){
	// echo "<br>";
	// echo "Nilai Tinggi = ".number_format($p_tinggi, dec())
        ...
        //===============================
        $nilai_rendah = $p_rendah * $prob_mar_rendah * $prob_apr_rendah *
					$prob_mei_rendah;
        if($show_perhitungan){
	// echo "<br>";
...
function get_jumlah_sum_atribut($db_object, $atribut, $minat){
	$sql = "SELECT SUM($atribut) FROM data_uji WHERE minat='$minat'";
	$res = $db_object->db_query($sql);
...
function get_jumlah_atribut($db_object, $atribut, $nilai, $minat){
	$sql = "SELECT COUNT(*) FROM data_uji WHERE $atribut='$nilai' AND minat='$minat'";
	$res = $db_object->db_query($sql);
	$row = $db_object->db_fetch_array($res);
	return $row[0];
}
...
function get_s2_populasi($db_object, $atribut, $minat, $x_params, $jml_params){
	$sql = "SELECT $atribut FROM data_uji WHERE minat='$minat'";
	$res = $db_object->db_query($sql);
	$sum_power = 0;
